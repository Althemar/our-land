...
namespace UnityEditor
{
	...
	internal class RuleTileEditor : Editor
	{
        ...
        static void RuleHexagonOnGUI(Vector3 center, HexMetrics metrics) {

            for (int i = 0; i <= 5; i++) {
                if (i != 5) {
                    Handles.DrawLine(center + metrics.GetCorner(i), center + metrics.GetCorner(i + 1));
                }
                else {
                    Handles.DrawLine(center + metrics.GetCorner(i), center + metrics.GetCorner(0));
                }
            }
		...
		internal static void RuleMatrixOnGUI(RuleTile tile, Rect rect, RuleTile.TilingRule tilingRule)
		{
            ...
            /*
            float outerRadius = rect.height / 4f;
            HexMetrics metrics = new HexMetrics(outerRadius);

            RuleHexagonOnGUI(rect.center, metrics);
            RuleHexagonOnGUI(new Vector3(rect.center.x + metrics.inn))
