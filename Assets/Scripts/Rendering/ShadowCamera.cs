...
public class ShadowCamera : MonoBehaviour {
    ...
    void OnRenderImage(RenderTexture src, RenderTexture dest) {
        ...
        entity = RenderTexture.GetTemporary(Screen.width, Screen.height, 0);
        entityCam.targetTexture = entity;

        blurMat.SetVector("_BlurSize", new Vector2(entity.texelSize.x * 3f, entity.texelSize.y * 3f));

        RenderTexture shadow = RenderTexture.GetTemporary(Screen.width, Screen.height, 0);
        RenderTexture tmp = RenderTexture.GetTemporary(Screen.width, Screen.height, 0);
